...
func officialregistryCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:    "officialregistry",
		Short:  "Manage official MCP registry operations",
		Long:   "Commands for importing and managing servers from official MCP registries",
		Hidden: true,
	}

	cmd.AddCommand(officialregistryImportCommand())

	return cmd
}

func officialregistryImportCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "import <url>",
		Short: "Import a server from an official registry URL",
...
Example:
  docker mcp officialregistry import https://registry.example.com/servers/my-server`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			return runOfficialregistryImport(cmd.Context(), args[0])
		},
	}
...
func runOfficialregistryImport(ctx context.Context, serverURL string) error {
	// Validate URL
